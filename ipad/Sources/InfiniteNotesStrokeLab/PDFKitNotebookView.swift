...
final class VectorPDFScrollView: UIScrollView, UIScrollViewDelegate {
    ...
    private var isRebuildingPageLayout = false
    ...
    func applySettings(settingsRevision: Int, workspaceRevision: Int) {
        ...
        if pdfFrames.indices.contains(target), zoomScale > 0 {
            ...
            let viewportCenter = viewportCenterInDocumentCoordinates()
            sourceAnchor = CGPoint(x: viewportCenter.x - oldPDF.minX, y: viewportCenter.y - oldPDF.minY)
        } else {
            sourceAnchor = nil
        }

        // UIScrollView applies its zoom as a transform to documentView. Changing
        // the frame of that transformed view, then assigning an unscaled
        // contentSize, can leave the scrollable range permanently truncated.
        // Return to an identity transform before rebuilding the document bounds,
        // then restore the previous zoom and source-PDF anchor.
        ...
        setZoomScale(restoredZoomScale, animated: false)
        layoutIfNeeded()
        updateContentInsets()
    ...
    func setDocument(_ document: PDFDocument?, sourceURL: URL?) {
        isRebuildingPageLayout = true
        unmountAllPages()
        // A new PDF can arrive while the old document is zoomed. Resetting first
        // prevents assigning frame/contentSize to a transformed zoom view.
        setZoomScale(1, animated: false)
    ...
    @objc private func displayLinkDidFire(_ link: CADisplayLink) {
        guard window != nil,
              model.appSettings.configuration.resolvedShowFPSInStatusBar else {
            fpsWindowStart = 0
            fpsFrameCount = 0
            if model.displayFPS != 0 { model.updateDisplayFPS(0) }
            return
        }
        ...
        if elapsed >= 0.5 {
            model.updateDisplayFPS(Double(fpsFrameCount) / elapsed)
            fpsWindowStart = link.timestamp
            fpsFrameCount = 0
        }
    }
    ...
    func goToPage(index: Int, animated: Bool) {
        ...
        let targetY = pageFrame.minY * zoomScale - 8
    ...
    func fitCurrentPage(animated: Bool) {
        ...
        let targetX = page.midX * clamped - bounds.width / 2
        let targetY = page.midY * clamped - bounds.height / 2
    ...
    private func viewportCenterInDocumentCoordinates() -> CGPoint {
    ...
    private func restoreViewportCenter(_ documentPoint: CGPoint, animated: Bool) {
        let targetX = documentPoint.x * zoomScale - bounds.width / 2
        let targetY = documentPoint.y * zoomScale - bounds.height / 2
        let minX = -contentInset.left
        let minY = -contentInset.top
        let maxX = max(minX, contentSize.width - bounds.width + contentInset.right)
        let maxY = max(minY, contentSize.height - bounds.height + contentInset.bottom)
    ...
    private func updateWorkingSet() {
        ...
        let visibleMidY = (contentOffset.y + bounds.height / 2) / zoomScale
